...
const THEME_PALETTE: Record<string, HexNodeTheme> = {
    ...
    '8': { main: '#451a03', light: '#78350f', dark: '#271a0c', stroke: '#d97706' }, 
...
const FloatingEffect: React.FC<{ effect: FloatingText; rotation: number; stackIndex: number }> = React.memo(({ effect, rotation, stackIndex }) => {
    ...
    const verticalOffset = stackIndex * 25; 
    ...
    useEffect(() => {
        ...
        new Konva.Tween({
            node: node, 
            y: -80, // Move up by 80px relative to starting point
            opacity: 0, 
            scaleX: 1.2, 
            scaleY: 1.2,
    ...
    return (
        ...
        <Group x={x} y={y - 20 - verticalOffset} listening={false} perfectDrawEnabled={false}>
            ...
            for (let r = centerHex.r - radius; r <= centerHex.r + radius; r++) {
                ...
                if (!isPlayerGrowing && (hex.structureType as string) !== 'VOID') {
                    ...
                    // Level 1.6: Vertical Limit (Arrow on Summit)
                        ...
                        // Find the "Summit" target (Highest maxLevel neighbor or center if high)
                        // Actually, objective is Reach Level 4. The highest point is likely the goal.
                        // Let's point to the highest level hex that is NOT occupied by player
                        // Simple heuristic: Point to (0,0) if it's high, or neighbors.
                ...
                items.push({
                    ...
                    props: {
                        ...
                        isTargetArrow: isArrow, // New Prop
