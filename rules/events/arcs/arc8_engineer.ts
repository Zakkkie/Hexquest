...
export const ARC8_EVENTS: Record<string, OverworldEvent> = {
  ...
  engineer_workshop: {
    ...
    nodes: {
      ...
      drill_in: {
        id: 'drill_in',
        text: 'Бур «Шершень» с пронзительным визгом вгрызается в сверхпрочный сплав инженерного замка. Раскалённые искры веером летят во все стороны, освещая тёмный коридор, но через минуту упорной работы древний механизм сдаётся с громким лязгом. Дверь медленно отъезжает в сторону. Вы внутри, но этот шум наверняка был слышен на несколько кварталов вокруг — стоит поторопиться.',
        choices: [
          {
            label: 'Быстро войти',
            action: 'GOTO_NODE',
      ...
      take_blueprint: {
        id: 'take_blueprint',
        text: 'Чертёж представляет собой невероятно сложную схему устройства, использующего энергию Пустоты. Вы не понимаете и половины обозначений, но центральный узел узнаваем: это портативный реактор. Как только вы снимаете свиток с креплений, в глубине комнаты что-то щёлкает, и загорается красная лампа — активировалась скрытая система безопасности. Гул сирен нарастает, нужно немедленно уходить.',
        choices: [
          {
