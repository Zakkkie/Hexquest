...
export const ARC10_EVENTS: Record<string, OverworldEvent> = {
  ...
  heartstone_decode: {
    ...
    nodes: {
      ...
      decode_with_scanner: {
        ...
        text: 'Вы направляете луч сканера на истёртую поверхность карты. Устройство издаёт серию коротких сигналов, обрабатывая данные, и внезапно на вашем дисплее вспыхивает детальная трёхмерная проекция окрестностей. Скрытые тропы, заваленные проходы и, самое главное, пульсирующая точка в центре — точное местоположение Сердца. Теперь путь ясен как никогда.',
        choices: [
          {
            label: 'Запомнить маршрут',
            action: 'CLOSE',
  ...
  heartstone_path: {
    ...
    nodes: {
      ...
      fix_bridge_drill: {
        id: 'fix_bridge_drill',
        text: 'Используя бур как импровизированный инструмент для забивки свай, вы укрепляете шаткие опоры моста. Гул мотора резонирует в скалах, пока вы вгоняете стальные штифты в твёрдую породу. Через некоторое время конструкция перестаёт дрожать под ногами. Мост теперь выдержит даже тяжёлую технику, а вы сэкономили кучу времени на обходном пути.',
        choices: [
          {
            label: 'Пройти по мосту',
            action: 'CLOSE',
