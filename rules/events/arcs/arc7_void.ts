  ...
  void_sample_found: {
    ...
    nodes: {
      ...
      safe_extract_scanner: {
        ...
        text: 'Сканер Стабильности анализирует частоту Пустоты, выводя на экран сложные графики резонанса. Вы аккуратно подстраиваете частоту вашего оборудования, создавая анти-фазу. Кристалл, лишившись своей энергетической привязки к реальности, сам выпадает в ваши руки, полностью нейтрализованный. Сканер перегревается и издаёт жалобный писк, но образец теперь безопасен для транспортировки.',
        choices: [
          {
            label: 'Взять образец',
            action: 'CLOSE',
      ...
      safe_extract_patch: {
        ...
        text: 'Вы обклеиваете область вокруг кристалла Лоскутами Реальности. Пространство начинает мерцать и застывает, словно под слоем прозрачного льда. Вы с усилием выламываете кристалл из земли — он всё ещё пульсирует внутри своей временной тюрьмы, но Лоскуты надёжно сдерживают разрушительную энергию, не давая ей коснуться вашей плоти.',
        choices: [
          {
            label: 'Взять образец',
            action: 'CLOSE',
